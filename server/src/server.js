...
app.post('/api/complaints', auth, roles('USER', 'ADMIN'), (req, res) => { const { title, description, location = {} } = req.body; if (!title || !description) return res.status(400).json({ success: false, message: 'Title and description are required' }); const result = analyze({ title, description }); const complaint = { _id: randomUUID(), complaintId: id('CMP', data.complaints.length + 1), createdBy: req.user._id, title, description, category: result.category, subCategory: result.subCategory, priority: result.priority, priorityScore: result.priority === 'HIGH' ? 58 : result.priority === 'MEDIUM' ? 31 : 12, priorityReason: 'Deterministic priority signal from complaint language', location: { building: location.building || 'Main Campus', floor: location.floor || '', room: location.room || '' }, department: result.department._id, status: 'ANALYZING', aiAnalysis: result, createdAt: now(), updatedAt: now() }; data.complaints.unshift(complaint); log('FALLBACK_ANALYSIS', 'Gemini unavailable; deterministic analysis applied', complaint, { category: result.category }); const assignment = assign(complaint); ok(res, { complaint: complaintView(complaint), assignment }, 'Complaint analyzed and routed'); });
...
app.delete('/api/admin/shifts/:id', auth, roles('ADMIN'), (req, res) => { const index = data.shifts.findIndex(s => s._id === req.params.id); if (index >= 0) data.shifts.splice(index, 1); ok(res, null, 'Shift removed'); });
